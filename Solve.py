...
from model.Unet import *  # 需要改动 2022.11.10
import matplotlib
...
class Solver(object):
    def __init__(self, configs):
        ...
        with self.writer:
            if not self.resume:
                inp = torch.randn([1, self.channels, 256, 256]).cuda() if self.cuda else torch.randn(
                    [1, self.channels, 256, 256])
    ...
    def train(self, epoch):
        ...
        #  logInfo生成日志信息表，“:.4f”为保留小数点后四位
        logInfo = 'Train at epoch [{}/{}] : mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, self.end_epoch,
                                                                                           mIoU, mAcc, allAcc)
        mIoU1 = ('{:.4f}'.format(mIoU))
        mAcc1 = ('{:.4f}'.format(mAcc))
        allAcc1 = ('{:.4f}'.format(allAcc))
        file_hand = open('train.txt', mode="a+")
        file_hand.write(mIoU1 + ',' + mAcc1 + ',' + allAcc1 + '\n')
        file_hand.close()
        print(logInfo)
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            ...
            logInfo = 'Val at epoch [{}/{}] : mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, self.end_epoch,
                                                                                             mIoU, mAcc, allAcc)
            ...
            file_hand.write(mIoU2 + ',' + mAcc2 + ',' + allAcc2 + '\n')
            file_hand.close()
            print(logInfo)
    ...
    def save_pred(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_dataloader):
                ...
                # 保存预测图片，像素值为0&1
                cv2.imwrite('test_output/test_output_1/{}.png'.format(path), saved_1,
                            [int(cv2.IMWRITE_JPEG_QUALITY), 95])
                # 保存预测图片，像素值为0&255
                cv2.imwrite('test_output/test_output_255/{}.png'.format(path), saved_255,
                            [int(cv2.IMWRITE_JPEG_QUALITY), 95])
...
if __name__ == "__main__":
    ...
    config = get_yaml('ConfigFiles/config-Unet.yaml')  # 调用配置文件,更换网络的地方   2022.11.10
    print(config)
