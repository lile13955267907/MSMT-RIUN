...
class FCN8s(nn.Module):
    ...
    def __init__(self, pretrained_net, n_class):
        ...
        self.deconv1 = nn.ConvTranspose2d(512, 512, kernel_size=3, stride=2, padding=1, dilation=1, output_padding=1)
        self.bn1     = nn.BatchNorm2d(512)
        self.attn_b1 = Attention_block(512, 512, 256)
        self.deconv2 = nn.ConvTranspose2d(512, 256, kernel_size=3, stride=2, padding=1, dilation=1, output_padding=1)
        self.bn2     = nn.BatchNorm2d(256)
        self.attn_b2 = Attention_block(256, 256, 128)
        self.deconv3 = nn.ConvTranspose2d(256, 128, kernel_size=3, stride=2, padding=1, dilation=1, output_padding=1)
        self.bn3     = nn.BatchNorm2d(128)
        self.deconv4 = nn.ConvTranspose2d(128, 64, kernel_size=3, stride=2, padding=1, dilation=1, output_padding=1)
        self.bn4     = nn.BatchNorm2d(64)
    ...
    def forward(self, x):
        ...
        # 从VGG中提取需要与后面进行特征融合的feature map
        # FCN8s为x3 x4
        output = self.pretrained_net(x)
        x5 = output['x5']                                # size=(N, 512, x.H/32, x.W/32)
        x4 = output['x4']                                # size=(N, 512, x.H/16, x.W/16)
        ...
        # 逐层进行卷积转置 在对应出进行element-wise add的特征融合
        # 得到对图片的预测
        score = self.relu(self.deconv1(x5))               # size=(N, 512, x.H/16, x.W/16)
        x4_   = self.attn_b1(score, x4)
        score = self.bn1(score + x4_)                      # element-wise add, size=(N, 512, x.H/16, x.W/16)
       
        score = self.relu(self.deconv2(score))            # size=(N, 256, x.H/8, x.W/8)
        x3_   = self.attn_b2(score, x3)
        score = self.bn2(score + x3_)                      # element-wise add, size=(N, 256, x.H/8, x.W/8)
